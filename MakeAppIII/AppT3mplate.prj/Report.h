...
public:
 ...
 ~Report() { }
  ...
  void operator() (bool printing = false);        // Determine the boundaries of the output medium then
                                                  // output the report
  void detNoLines(AppT3mplateView& vw);               // Performs a dummy printer output to determine the
                                                  // number of lines on each page
  void detWraps(  AppT3mplateView& vw);               // Wrap each entity in the report body

  void detNoPages(AppT3mplateView& vw);               // Performs a dummy printer output to determine the
