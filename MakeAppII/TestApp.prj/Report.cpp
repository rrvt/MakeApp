...
void Report::detNoLines(TestAppView& vw) {
int i;
int mxPgs;
...
void Report::detWraps(TestAppView& vw) {
...
void Report::detNoPages(TestAppView& vw) {int nLns;   create();   vw.trialRun(nLns, maxPages);}
...
int Report::header() {
  ...
  notePad << nBold << nFSize(160) << _T("Test App") << nFont << nFont;
