// ExpandableX:  Array of Pointers Extended(only)
...
The DatumPtrX template defines a pointer to the Datum.  A DatumX is the item that is stored in the array.
This leads to some efficiency since when moving items in the array, only a pointer is moved.  However, it
also introduces some complexity.  For the most part, ExpandableX manages allocating and deallocating the
...
the ExpandableX object.  This then requires a class with a minimal set of boolean operations to perform
...
The operations supported by an ExpandableX array where the declaration is:

  typedef DatumPtrX<Datum, KeyClass> DatumX;                // Usually defined just before iterator

  class Xyz {
  ExpandableX<Datum, KeyClass, DatumX, 2> data;
  Datum                         datum;
        ,,,
...
public:
  ...
  ExpandableX();                      // Constructor & Destructor
 ~ExpandableX();

 ExpandableX& operator= (ExpandableX& e);                               // copy the whole array
...
private:
  ...
  void   copy(ExpandableX& e);                  // Copy array e to this array
...
template <class Datum, class Key, class DatumX, const int n>
ExpandableX<Datum, Key, DatumX, n>::ExpandableX() : endN(0), tblN(n > 0 ? n : 1)
...
template <class Datum, class Key, class DatumX, const int n>
ExpandableX<Datum, Key, DatumX, n>::~ExpandableX()
...
template <class Datum, class Key, class DatumX, const int n>
ExpandableX<Datum, Key, DatumX, n>& ExpandableX<Datum, Key, DatumX, n>::operator= (ExpandableX& e)
...
template <class Datum, class Key, class DatumX, const int n>
DatumX& ExpandableX<Datum, Key, DatumX, n>::operator[] (int i) {
  if (i >= tblN) expand(i);
  if (i >= endN) endN = i+1;
  return tbl[i];
  }
...
template <class Datum, class Key, class DatumX, const int n>
Datum* ExpandableX<Datum, Key, DatumX, n>::operator+= (Datum* r)
...
template <class Datum, class Key, class DatumX, const int n>
Datum& ExpandableX<Datum, Key, DatumX, n>::nextData()
...
template <class Datum, class Key, class DatumX, const int n>
Datum& ExpandableX<Datum, Key, DatumX, n>::getData(int i) {
...
template <class Datum, class Key, class DatumX, const int n>
bool ExpandableX<Datum, Key, DatumX, n>::operator() (int x, Datum& d)
...
template <class Datum, class Key, class DatumX, const int n>
bool ExpandableX<Datum, Key, DatumX, n>::del(Datum* p) {
int i;
...
template <class Datum, class Key, class DatumX, const int n>
bool ExpandableX<Datum, Key, DatumX, n>::del(int x) {
int i;
...
template <class Datum, class Key, class DatumX, const int n>
Datum* ExpandableX<Datum, Key, DatumX, n>::bSearch(Key& key) {
int     beg  = 0;
int     end  = endN;
int     last = -1;
int     i;
...
template <class Datum, class Key, class DatumX, const int n>
Datum* ExpandableX<Datum, Key, DatumX, n>::find(Key& key) {
int i;
...
template <class Datum, class Key, class DatumX, const int n>
Datum* ExpandableX<Datum, Key, DatumX, n>::insertSorted(Datum* p) {
DatumX xNode;
DatumX nextNode;
int    i;
...
template <class Datum, class Key, class DatumX, const int n>
void ExpandableX<Datum, Key, DatumX, n>::expand(int i) {
DatumX* p      = tbl;
DatumX* q      = tbl;
int     nItems = tblN;
int     j;
...
template <class Datum, class Key, class DatumX, const int n>
void ExpandableX<Datum, Key, DatumX, n>::copy(ExpandableX& e) {
...
template <class Datum, class Key, class DatumX, const int n>
void ExpandableX<Datum, Key, DatumX, n>::freeAllNodes() {
